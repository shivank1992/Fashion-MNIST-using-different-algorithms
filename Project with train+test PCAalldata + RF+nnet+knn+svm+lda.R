#Install packages
list.of.packages <- c("keras","tensorflow","class","ggplot2","knitr","readr","caret","ggthemes","plotly","graphics","forcats")
...
fashion.train <- read.csv("fashion-mnist_train.csv")
fashion.train2<- fashion.train[,-1]
dim(fashion.train)

# set seed for reproducing the partition
set.seed(1)
training.index <- createDataPartition(fashion.train$label, p = 0.6, list = FALSE)
train <- fashion.train[training.index, ]
valid <- fashion.train[-training.index, ]
...
label.factor <- as.factor(fashion.train$label)
...
sample_plots = sample(1:nrow(fashion.train),35) %>% map(~ {
  plot_data = cbind(xy_axis, fill = as.data.frame(t(fashion.train[.x, -1]))[,1]) 
...
label_count_plot_train <- ggplot(data = fashion.train, aes(x = label, fill = as.factor(label))) + 
...
plot(results$num, results$cum, type = "b", xlim = c(0,20),
     main = "Variance Explained by Top 10 Components",
     xlab = "Number of Components", ylab = "Variance Explained")


#Replace train
train.score <- as.matrix(trainnzv) %*% train.pc$rotation[,1:10]
...
valid.score <- as.matrix(validnzv) %*% train.pc$rotation[,1:10]
...
model.accuracypc10<- setNames(data.frame(matrix(ncol = 3, nrow = 0)), 
                          c("model", "accuracy_pc10","timeTaken_pc10"))
...
model.accuracypc10['rf',] <- c('rf', rf_cm$overall[1],as.numeric(t, units = "secs"))
...
#Tune SVM
start.time <- Sys.time()
svm_tune <- tune(svm, label ~ ., data=train.pca, 
...
model.accuracypc10['svm',] <- c('svm', svm_cm$overall[1],as.numeric(t, units = "secs"))
...
model.accuracypc10['lda',] <- c('lda', lda_cm$overall[1],as.numeric(t, units = "secs"))
...
TrainingParameters <- trainControl(method = "cv", number = 3,allowParallel = T)
grid_nn <- expand.grid(.size = c(120,125,130,135),
                       .decay = 0)

grid_nn

nn2 <- train(label~., data = train.pca,
...
pred_tunednnet <- predict(nn2,valid.pca,type="class")
nnet_cm <- confusionMatrix(factor(pred_tunednnet), 
                           valid.pca$label,
...
model.accuracypc10['nnet',] <- c('nnet', nnet_cm$overall[1],as.numeric(t, units = "secs"))
...
model.accuracypc10['knn',] <- c('knn', knn_cm$overall[1],as.numeric(t, units = "secs"))
